...
badFlag(A,'-fcombine-stack-adjustments') :- ft(ft7,A,0), ft(ft34,
	A,0).
badFlag(A,'-fguess-branch-probability') :- ft(ft31,A,0), ft(ft42,
	A,0), large_ft(A,ft3), large_ft(A,ft11), qt(A,ft47,3).
badFlag(A,'-fivopts') :- ft(ft4,A,0), ft(ft10,A,0), ft(ft14,A,
	0), large_ft(A,ft3).
...
badFlag(A,'-ftree-reassoc') :- ft(ft14,A,0), large_ft(A,ft6).
badFlag(A,'-ftree-sra') :- ft(ft43,A,0).
...
badFlag(A,'-freschedule-modulo-scheduled-loops') :- ft(ft6,A,
	0).
badFlag(A,'-ftree-loop-distribute-patterns') :- ft(ft6,A,0).
...
badFlag(A,'-ftree-ccp') :- ft(ft7,A,0).
