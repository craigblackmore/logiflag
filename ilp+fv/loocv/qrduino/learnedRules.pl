...
badFlag(A,'-ftree-loop-distribute-patterns') :- small_ft(A,ft2).
badFlag(A,'-ftree-reassoc') :- large_ft(A,ft6).
badFlag(A,'-fsection-anchors') :- ft(ft23,A,0).
...
badFlag(A,'-foptimize-sibling-calls') :- ft(ft4,A,0), ft(ft43,
	A,0), large_ft(A,ft7).
...
badFlag(A,'-ftree-ter') :- ft(ft34,A,0).
...
badFlag(A,'-fsched-spec-load') :- ft(ft7,A,0).
...
badFlag(A,'-freorder-blocks') :- qt(A,ft6,1).
