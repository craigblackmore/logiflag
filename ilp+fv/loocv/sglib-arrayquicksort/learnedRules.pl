...
badFlag(A,'-fcprop-registers') :- ft(ft23,A,0).
...
badFlag(A,'-fsection-anchors') :- ft(ft7,A,0), ft(ft23,A,0).
...
badFlag(A,'-foptimize-sibling-calls') :- ft(ft4,A,0), ft(ft10,
	A,0), large_ft(A,ft7).
...
badFlag(A,'-ftree-vrp') :- ft(ft4,A,0), ft(ft6,A,0).
...
badFlag(A,'-falign-functions') :- ft(ft29,A,0).
badFlag(A,'-fbranch-count-reg') :- ft(ft31,A,0).
badFlag(A,'-fcprop-registers') :- ft(ft10,A,0).
