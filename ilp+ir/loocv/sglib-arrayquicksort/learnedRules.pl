...
badFlag(A,'-fcprop-registers') :- stmt_flag(A,B,C,has_volatile_ops).
...
badFlag(A,'-fsel-sched-pipelining') :- expr_code2(A,B,array_type).
...
badFlag(A,'-fif-conversion') :- stmt_code(A,B,C,gimple_cond),
	stmt_flag(A,B,D,has_volatile_ops).
