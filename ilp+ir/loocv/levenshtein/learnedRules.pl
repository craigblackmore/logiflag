...
badFlag(A,'-faggressive-loop-optimizations') :- stmt_code(A,B,
	C,gimple_assign), stmt_flag(A,B,C,has_volatile_ops).
...
badFlag(A,'-fschedule-insns') :- expr_class(A,B,C,constant), 
	phi_arg(A,B,D,C).
...
badFlag(A,'-fsched-spec-insn-heuristic') :- stmt_code(A,B,C,gimple_assign).
