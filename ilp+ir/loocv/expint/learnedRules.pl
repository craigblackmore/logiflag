...
badFlag(A,'-ftree-copyrename') :- expr_int_size(A,B,C,64).
...
badFlag(A,'-fipa-cp-clone') :- expr_class(A,B,C,reference), call_num_args(A,
	B,D,0).
badFlag(A,'-fipa-pure-const') :- stmt_code(A,B,C,gimple_assign),
	stmt_flag(A,B,D,has_volatile_ops).
...
badFlag(A,'-fcaller-saves') :- stmt_code(A,B,C,gimple_assign),
	expr_code_len(A,B,D,3).
badFlag(A,'-fdevirtualize') :- expr_code_len(A,B,C,3).
