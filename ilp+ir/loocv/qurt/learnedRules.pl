...
badFlag(A,'-frename-registers') :- expr_int_size(A,B,C,64).
...
badFlag(A,'-fforward-propagate') :- expr_int_size(A,B,C,8).
badFlag(A,'-fgcse-after-reload') :- stmt_code(A,B,C,gimple_assign).
badFlag(A,'-fivopts') :- stmt_code(A,B,C,gimple_cond), expr_code_len(A,
	B,D,3).
...
badFlag(A,'-fselective-scheduling2') :- expr_code2(A,B,array_ref).
...
badFlag(A,'-foptimize-sibling-calls') :- stmt_code(A,B,C,gimple_cond),
	stmt_flag(A,B,D,has_volatile_ops).
badFlag(A,'-fsched-pressure') :- stmt_flag(A,B,C,has_volatile_ops).
...
badFlag(A,'-ftree-ch') :- stmt_code(A,B,C,gimple_assign), stmt_code(A,
	...
	B,F,3).
badFlag(A,'-ftree-loop-distribution') :- expr_code_len(A,B,C,
	3).
...
badFlag(A,'-fsched-last-insn-heuristic') :- stmt_code(A,B,C,gimple_assign),
	stmt_code(A,B,D,gimple_call), stmt_code(A,B,E,gimple_cond).
