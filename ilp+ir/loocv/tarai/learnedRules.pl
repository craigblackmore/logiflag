...
badFlag(A,'-fivopts') :- stmt_code(A,B,C,gimple_assign), stmt_code(A,
...
badFlag(A,'-fguess-branch-probability') :- stmt_code(A,B,C,gimple_assign),
	stmt_code(A,B,D,gimple_call), stmt_code(A,B,E,gimple_cond),
	stmt_flag(A,B,C,has_volatile_ops), call_num_args(A,B,
	D,2).
...
badFlag(A,'-fipa-pure-const') :- stmt_code(A,B,C,gimple_assign),
	stmt_flag(A,B,D,has_volatile_ops).
badFlag(A,'-fivopts') :- stmt_code(A,B,C,gimple_assign), stmt_code(A,
	B,D,gimple_call), stmt_flag(A,B,E,has_volatile_ops).
...
badFlag(A,'-fsplit-ivs-in-unroller') :- stmt_flag(A,B,C,has_volatile_ops).
