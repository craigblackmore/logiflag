...
badFlag(A,'-fcprop-registers') :- call_num_args(A,B,C,5).
...
badFlag(A,'-ftree-loop-distribute-patterns') :- stmt_code(A,B,
	C,gimple_assign), stmt_code(A,B,D,gimple_cond).
badFlag(A,'-ftree-reassoc') :- stmt_code(A,B,C,gimple_cond).
...
badFlag(A,'-fsection-anchors') :- stmt_code(A,B,C,gimple_cond),
	stmt_flag(A,B,D,has_volatile_ops).
badFlag(A,'-ftree-vrp') :- stmt_code(A,B,C,gimple_assign), stmt_flag(A,
...
badFlag(A,'-fipa-pure-const') :- stmt_code(A,B,C,gimple_label).
badFlag(A,'-fipa-sra').
...
badFlag(A,'-ftree-builtin-call-dce') :- expr_class(A,B,C,reference),
	expr_code2(A,B,array_ref).
...
badFlag(A,'-fcaller-saves') :- expr_code_len(A,B,C,3).
badFlag(A,'-fdevirtualize') :- stmt_code(A,B,C,gimple_assign),
	expr_code_len(A,B,D,3).
badFlag(A,'-fgcse') :- stmt_code(A,B,C,gimple_cond), expr_code_len(A,
	B,D,3).
badFlag(A,'-fipa-pta').
...
badFlag(A,'-ftree-ch') :- stmt_code(A,B,C,gimple_assign), stmt_code(A,
	B,D,gimple_cond), stmt_code(A,B,E,gimple_call), expr_code_len(A,
	B,F,3).
...
badFlag(A,'-fipa-cp') :- stmt_code(A,B,C,gimple_assign), stmt_flag(A,
	B,C,has_volatile_ops).
badFlag(A,'-fprefetch-loop-arrays') :- stmt_code(A,B,C,gimple_call),
	stmt_flag(A,B,D,has_volatile_ops).
badFlag(A,'-fsched-group-heuristic').
badFlag(A,'-fsplit-ivs-in-unroller') :- stmt_code(A,B,C,gimple_assign),
	stmt_flag(A,B,C,has_volatile_ops).
badFlag(A,'-ftree-ccp') :- stmt_flag(A,B,C,has_volatile_ops).
badFlag(A,'-ftree-scev-cprop') :- stmt_code(A,B,C,gimple_assign),
	stmt_flag(A,B,C,has_volatile_ops).
